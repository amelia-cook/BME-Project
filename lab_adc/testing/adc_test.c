...
/* ================================================================== */
/*  PHASE 3 TESTS — async_adc_tests                                  */
/* ================================================================== */

/*
 * Async path should produce the same cycle count as the sync path.
 * The observable interface is identical; only the internal mechanism differs.
 * ADC_ASYNC_DONE_NOTICE must fire instead of (or in addition to)
 * ADC_SAMPLE_COMPLETE_NOTICE.
 */
ZTEST(async_adc_tests, test_p3_01_async_produces_same_cycle_count)
{
    set_differential_sine(adc_emul_dev, 10, 2000, 2500);
    start_main(500);
    ...
    /* Async-specific notice must fire */
    uint32_t events = k_event_wait(&program_test_events,
                                   ADC_ASYNC_DONE_NOTICE,
                                   true, K_MSEC(3000));
    zassert_true(events & ADC_ASYNC_DONE_NOTICE,
        "ADC_ASYNC_DONE_NOTICE never fired — did you add ADC_ASYNC_COMPLETE() "
        "to the k_poll signal handler?");

    /* Cycle count should still be ~20 */
    assert_cycles_computed(20, 2);
}

/*
 * Verify ADC_ASYNC_DONE_NOTICE fires within a reasonable timeout
 * (acquisition time + 500 ms headroom).
 */
ZTEST(async_adc_tests, test_p3_02_async_signal_fires)
{
    set_differential_sine(adc_emul_dev, 10, 2000, 2500);
    start_main(500);
    ...
    zassert_true(events & ADC_ASYNC_DONE_NOTICE,
        "ADC_ASYNC_DONE_NOTICE did not fire within %d ms", timeout_ms);
}

/*
 * Timeout mechanism: if the async acquisition never completes,
 * ADC_ASYNC_TIMEOUT_NOTICE should fire and state should go to ERROR.
 *
 * We simulate a hung ADC by installing a callback that never calls
 * ADC_ACTION_FINISH on the emulator.  The student's timeout logic
 * (k_poll with timeout) should catch this.
 *
 * NOTE: This test is only meaningful if the student implements a
 * finite timeout on their k_poll call (as required by the spec).
 */
ZTEST(async_adc_tests, test_p3_03_async_timeout_mechanism)
{
    /*
     * Don't configure the AIN1/AIN2 emulator callbacks — the emulator
     * will return an error on adc_read_async, which should push the
     * student to the ERROR state.
     *
     * If the student's implementation uses a k_poll timeout:
     *   set_differential_sine() but never post SAMPLE_COMPLETE_EVENT
     *   from the callback — that requires patching the student macro,
     *   which is out of scope.  Instead we verify the error path via
     *   a bad adc_read_async return code.
     */

    /* Leave AIN1/AIN2 unconfigured — adc_read_async should fail */
    start_main(500);
    simulate_button_click(&sample_button);
    ...
    /*
     * Either ADC_ASYNC_TIMEOUT_NOTICE fires, or ERROR_TEST_NOTICE fires —
     * accept either as evidence the student handles the failure path.
     */
    uint32_t events = k_event_wait(&program_test_events,
                                   ADC_ASYNC_TIMEOUT_NOTICE | ERROR_TEST_NOTICE,
                                   true, K_MSEC(5000));
    zassert_true(events & (ADC_ASYNC_TIMEOUT_NOTICE | ERROR_TEST_NOTICE),
        "Neither timeout notice nor error state observed after hung async ADC");

    /* error_led should be on */
    k_msleep(100);
    assert_led_on(&error_led, "error (after async timeout)");
}

/*
 * After async acquisition completes, state returns to IDLE.
 * Same verification strategy as test_p2_04.
 */
ZTEST(async_adc_tests, test_p3_04_async_returns_to_idle)
{
    set_differential_sine(adc_emul_dev, 10, 2000, 2500);
    start_main(500);
    ...
    uint32_t events = k_event_wait(&program_test_events,
                                   ADC_ASYNC_DONE_NOTICE,
                                   true, K_MSEC(3000));
    zassert_true(events & ADC_ASYNC_DONE_NOTICE, "Async acquisition never completed");
    ...
    events = k_event_wait(&program_test_events,
                          ...
                          true, K_MSEC(300));
    zassert_true(events & ADC_READ_TRIGGERED_NOTICE,
        "read_button not responsive after async SAMPLE→IDLE transition");
}

/*
 * Heartbeat continues at 1 Hz throughout async SAMPLE state.
 */
ZTEST(async_adc_tests, test_p3_05_heartbeat_unaffected)
{
    set_differential_sine(adc_emul_dev, 10, 2000, 2500);
    start_main(500);
