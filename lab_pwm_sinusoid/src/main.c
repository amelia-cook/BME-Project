...
#define SAMPLE_EVENT                    BIT(4)
#define SAMPLE_COMPLETE_EVENT           BIT(5)
#define BLINKING_TIME_MS                5000
#define BUFFER_ARRAY_LEN                800
#define SAMPLE_INTERVAL                 2500
#define WORK_QUEUE_NAP_TIME_MS          1
...
const struct gpio_dt_spec sample_button = GPIO_DT_SPEC_GET(DT_ALIAS(samplingbutton), gpios);
...
const struct gpio_dt_spec sinusoidal_led = GPIO_DT_SPEC_GET(DT_ALIAS(sinusoidal), gpios);
...
void pwm_work_handler(struct k_work *pwm_work);
...
static struct gpio_callback sample_button_cb;
...
static const struct adc_dt_spec diff_vadc = ADC_DT_SPEC_GET_BY_ALIAS(diffadc);
...
struct adc_sequence_options options = {
        ...
        .interval_us = SAMPLE_INTERVAL,
        .callback = adc_callback,
};

int index = 0;
int16_t diff_buf[BUFFER_ARRAY_LEN];
struct adc_sequence diff_sequence = {
        .options = &options,
        .buffer = &diff_buf,
        .buffer_size = sizeof(diff_buf),
};
...
static void sample_entry(void *o);
static void sample_run(void *o);
static void sample_exit(void *o);

/* DEFINE STATES FOR STATE MACHINE */
enum state { INIT, RESET, IDLE, SLEEP, READING, BLINKING, ERROR, SAMPLE };
...
static const struct smf_state states[] = {
    ...
    [SAMPLE] = SMF_CREATE_STATE(sample_entry, sample_run, sample_exit, NULL, NULL),
};
...
enum adc_action adc_callback(const struct device *dev, const struct adc_sequence *sequence, uint16_t sampling_index) {
    if (sampling_index == BUFFER_ARRAY_LEN -1) {
        k_event_post(&button_events, SAMPLE_COMPLETE_EVENT);
...
void pwm_work_handler(struct k_work *pwm_work) {
    ...
    int32_t val_mv = diff_buf[index];
    ret = adc_raw_to_millivolts_dt(&adc_vadc, &val_mv);
    if (ret < 0) {
        LOG_ERR("Buffer cannot be converted to mV; returning raw buffer value.");
    }
    
    float dutycycle = (val_mv+1000.0)/20.0;
    
    /* SET PWM PULSE */
    err = pwm_set_pulse_dt(&pwm2, pwm2.period/(100/dutycycle));
    if (err) {
    ...
    index++;
    if (index == BUFFER_ARRAY_LEN - 1) {
        LOG_INF("index = %d", index);
        index = 0;
        
        /* SET PWM PULSE */
        err = pwm_set_pulse_dt(&pwm2, 0);
        if (err) {
            LOG_ERR("Could not set pwm1 driver.");
        }
    }
...
static void init(void *o) {
    ...
    if (!device_is_ready(diff_vadc.dev)) {
        LOG_ERR("ADC controller device(s) not ready");
        smf_set_terminate(SMF_CTX(&s_context), -1);
    }
    ...
    if (err < 0) {
        LOG_ERR("Cannot configure read button.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
    err = gpio_pin_configure_dt(&sample_button, GPIO_INPUT);
    if (err < 0) {
        LOG_ERR("Cannot configure sample button.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
    ...
    // sample
    err = gpio_pin_interrupt_configure_dt(&sample_button, GPIO_INT_EDGE_TO_ACTIVE); 
    if (err < 0) {
        LOG_ERR("Cannot attach callback to sw1.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
    gpio_init_callback(&sample_button_cb, sample_button_callback, BIT(sample_button.pin));
    err = gpio_add_callback_dt(&sample_button, &sample_button_cb);
    if (err < 0) {
    ...
    if (err < 0) {
        LOG_ERR("Cannot configure blinker LED.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
    err = gpio_pin_configure_dt(&sinusoidal_led, GPIO_OUTPUT_INACTIVE);
    if (err < 0) {
        LOG_ERR("Cannot configure sinusoidal LED.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
    ...
    err = adc_channel_setup_dt(&diff_vadc);
    if (err < 0) {
        LOG_ERR("Could not setup ADC channel (%d)", err);
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
...
static void idle_run(void *o) {
    uint32_t events = k_event_wait(&button_events, READ_EVENT | SLEEP_EVENT | RESET_EVENT | SAMPLE_EVENT, true, K_FOREVER);
    ...
    if (events & SAMPLE_EVENT) {
        LOG_INF("Sample button pressed");
        smf_set_state(SMF_CTX(&s_context), &states[SAMPLE]);
    }
}
...
static void idle_exit(void *o) {
    ...
    err = gpio_pin_interrupt_configure_dt(&sample_button, GPIO_INT_DISABLE); 
    if (err < 0) {
        LOG_ERR("Cannot attach callback to sw0.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
...
static void reading_run(void *o) {
    ...
    s_context.dutycycle = (s_context.millivolts / max_v) * 100;
    s_context.brightness = s_context.dutycycle;
    LOG_INF("Mapped duty cycle: %f", (double)s_context.dutycycle);
}
...
static void blinking_run(void *o) {
    ...
    /* SET PWM PULSE */
    err = pwm_set_pulse_dt(&pwm1, pwm1.period/(100/s_context.dutycycle));
    if (err) {
        LOG_ERR("Could not set pwm1 driver.");
    }
...
static void sample_run(void *o) {
    ...
    /* TURN ON LED */
    err =  gpio_pin_set_dt(&sinusoidal_led, 1);
    if (err < 0) {
        LOG_ERR("Failed to set sinusoidal LED.");
        smf_set_terminate(SMF_CTX(&s_context), err);
    }
    
    ret = adc_read_async(diff_vadc.dev, &diff_sequence, NULL);
    if (ret < 0) {
        LOG_ERR("Could not read (%d)", ret);
        smf_set_state(SMF_CTX(&s_context), &states[ERROR]);
        return;
